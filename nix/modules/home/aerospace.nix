# AeroSpace window manager. Package is installed at the system level via
# nix-darwin (see nix/modules/packages.nix) so the .app bundle gets linked
# into /Applications. The launchd user agent is managed by HM
# (programs.aerospace.launchd.enable) — that nullifies AeroSpace's own
# `start-at-login` / `after-login-command` and uses
# `org.nix-community.home.aerospace` instead.
#
...
{ ... }:
{
  programs.aerospace = {
    ...
    settings = {
      ...
      # Slack sometimes steals focus (during huddles)
      ...
      gaps = {
        ...
        # we are trying to match the notch size from the built-in retina display
        # and match sketchybar across both
        # unfortunately the notch size differs across 14 and 16 inch
        # laptops
        # notch heights:
        # Macbook 14 (M1)
        #   1024x 665          : 22
      ...
      mode.main.binding = {
        ...
        cmd-ctrl-1 = [
          "exec-and-forget \${XDG_CONFIG_HOME}/aerospace/make-frontmost.sh com.apple.finder"
          "exec-and-forget \${XDG_CONFIG_HOME}/aerospace/position-frontmost.sh center"
        ];
        cmd-ctrl-2 = [
      ...
      #
      # Find bundle ids and windows names
      ...
      on-window-detected = [
        ...
        { "if".app-id = "com.apple.Safari"; run = "move-node-to-workspace b"; }
        {
          ...
          # arc gets it's own workspace onyl because
          # it has wrong corner radius for macOS 26,
          # will probably never be re-compiled to match
          # and it's ugly when being behind another browser
  ...
  # Helper scripts referenced from the cmd-ctrl-1 / cmd-ctrl-2 bindings.
  # The HM aerospace module doesn't have a script option, so deploy these
  # via xdg.configFile.
  xdg.configFile = {
    "aerospace/make-frontmost.sh" = {
      source = ./aerospace/scripts/make-frontmost.sh;
      executable = true;
    };
    "aerospace/position-frontmost.sh" = {
      source = ./aerospace/scripts/position-frontmost.sh;
      executable = true;
    };
  };
}
