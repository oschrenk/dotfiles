{ pkgs, ... }:
let
  extrakto = pkgs.tmuxPlugins.extrakto;
in
{
  programs.tmux = {
    enable = true;
    ...
    extraConfig = ''
      ...
      bind-key -n C-Tab run-shell "${extrakto}/share/tmux-plugins/extrakto/scripts/open.sh #{pane_id}"
      ...
      set-hook -g session-closed 'run-shell "sketchybar --trigger tmux_windows_update"'
      set-hook -g client-detached 'run-shell "sketchybar --trigger tmux_windows_update"'
    '';
  };

  xdg.configFile = {
    "tmux/tmux-dirs.sh" = {
      source = ./tmux/scripts/tmux-dirs.sh;
      executable = true;
    };
