...

{
  programs.ripgrep = {
    enable = true;
    # Each list entry becomes one line of ~/.config/ripgrep/ripgreprc
    # (joined via lib.concatLines). Comment lines are valid in ripgrep's
    # config format, so they round-trip cleanly.
    arguments = [
