...
let
  ...
  makeHealthcheckScript =
    timestampFile: maxAge:
    pkgs.writeShellScript "backup-healthcheck" ''
...
in
{
  ...
  config = lib.mkIf (cfg.checks != { }) {
    ...
    systemd.services = lib.mapAttrs' (
      ...
      let
        effectivePath =
          if check.timestampFile != null then check.timestampFile else "${cfg.statusDir}/${name}";
      in
      lib.nameValuePair "backup-healthcheck-${name}@" {
        serviceConfig = {
          Type = "simple";
