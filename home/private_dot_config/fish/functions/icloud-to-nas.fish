...
function icloud-to-nas
    ...
    function _itn_log
        printf "%s[%s]%s %s\n" $C_GREEN (date +%H:%M:%S) $C_NC "$argv"
    end

    function _itn_warn
        printf "%s[%s] WARN:%s %s\n" $C_YELLOW (date +%H:%M:%S) $C_NC "$argv"
    end

    function _itn_err
        printf "%s[%s] ERROR:%s %s\n" $C_RED (date +%H:%M:%S) $C_NC "$argv" >&2
    end

    function _itn_human_size
        set -l bytes $argv[1]
    ...
    function _itn_file_size_bytes
        stat -f%z "$argv[1]" 2>/dev/null; or echo 0
    end

    function _itn_is_downloaded
        set -l blocks (stat -f%b "$argv[1]" 2>/dev/null; or echo 0)
        test "$blocks" -gt 0
    end

    function _itn_is_migrated
        grep -qFx "$argv[1]" "$PROGRESS_LOG" 2>/dev/null
    end

    function _itn_mark_migrated
        echo "$argv[1]" >>"$PROGRESS_LOG"
    end

    function _itn_download_and_wait
        set -l file $argv[1]
        set -l size $argv[2]
        ...
        if test $timeout -gt $MAX_POLL_TIMEOUT
            set timeout $MAX_POLL_TIMEOUT
        end
        ...
        while not _itn_is_downloaded "$file"
            ...
            printf "\r  %siCloud → disk... %ds%s  " $C_DIM $elapsed $C_NC
        end
        printf "\r                              \r"
        return 0
    end
    ...
    function _itn_process_file
        ...
        if _itn_is_migrated "$src"
            _itn_log "  {$C_DIM}skip{$C_NC} (already done)"
            return 0
        end
        ...
        if test -f "$dst"
            ...
            if test "$dst_size" = "$size"
                _itn_log "  {$C_DIM}skip{$C_NC} (already on NAS)"
                _itn_mark_migrated "$src"
                return 0
            end
        end

        if test "$DRY_RUN" = true
            _itn_log "  {$C_YELLOW}dry-run{$C_NC} $human"
            return 0
        end
        ...
        _itn_log "  {$C_BLUE}↓{$C_NC} iCloud → disk ($human)"
        if not _itn_download_and_wait "$src" $size
        ...
        _itn_log "  {$C_GREEN}✓{$C_NC} evicted local copy"
    ...
    if test "$DRY_RUN" = true
        _itn_log "{$C_YELLOW}DRY RUN — no files will be modified{$C_NC}"
    end
    echo ""
    ...
    while read -l file
        ...
        if _itn_process_file "$file" "$nas_dest/$rel_path"
            if _itn_is_migrated "$file"
